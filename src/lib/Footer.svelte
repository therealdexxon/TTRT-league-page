...
<footer bind:this={el}>
						...
						{#if child.label != "Managers" || managers.length > 0}
							<li>
								{#if child.label === "League Rules" || child.label === "Go to Sleeper"}
									<!-- Styled as navLink but opens in new tab -->
									<div
										class="navLink"
										on:click={() => openLinkInNewTab(child.dest)}
										role="link"
										tabindex="0"
										on:keydown={(e) => {
											if (e.key === 'Enter' || e.key === ' ') openLinkInNewTab(child.dest);
										}}
									>
										{child.label}
									</div>
	...
	<span class="copyright"
		>&copy; 2021 - {year}
		<a class="navLink" href="https://github.com/nmelhado/league-page" target="_blank" rel="noopener noreferrer">League Page</a>
	</span>
	<br />
	<span class="creator">
		Built by
		<a class="navLink" href="http://www.nmelhado.com/" target="_blank" rel="noopener noreferrer">Nicholas Melhado</a
		><br />
	</span>
	Love League Page? Please consider
	<a
		class="navLink"
		href="https://www.buymeacoffee.com/nmelhado"
		target="_blank"
		rel="noopener noreferrer"
		>donating</a
	>
